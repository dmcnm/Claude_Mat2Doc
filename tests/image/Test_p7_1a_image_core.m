    ...
    methods (Test)
        ...
        function test_factory_dispatch_to_stubs(testCase)
            ...
            for k = 1:size(cases, 1)
                ...
                result = [];
                try
                    result = mat2doc.image.Image.from_blob(blob);
                catch ME
                    caught = ME;
                end
                switch kind
                    case 'parse'
                        % GIF rows now parse to a real Image -- no error expected.
                        testCase.verifyEmpty(caught, ...
                            sprintf('%s must PARSE to a real Gif (P7-1b un-stub), not raise', lbl));
                        testCase.verifyEqual(string(result.content_type), "image/gif", ...
                            sprintf('%s must dispatch to the real Gif parser', lbl));
                    case 'eof'
                        testCase.assertNotEmpty(caught, ...
                            sprintf('%s must dispatch to the real parser and hit EOF', lbl));
                        testCase.verifyEqual(caught.identifier, 'mat2doc:UnexpectedEndOfFileError', ...
