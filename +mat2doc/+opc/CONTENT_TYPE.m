...
% CONTENT_TYPE Content type URIs (like MIME-types) that specify a part's format.
%
%   Access as mat2doc.opc.CONTENT_TYPE.WML_DOCUMENT_MAIN etc. Mirrors the
%   Python class attributes (class CONTENT_TYPE with class-level constants). Each
%   value is transcribed verbatim from the docx source; the several Python
%   implicit-string-concatenation entries (a value split across two source lines
%   for line length) are joined into the single string they denote.
%
%   Example:
%       disp(mat2doc.opc.CONTENT_TYPE.PNG)   % "image/png"
%
%   Ported from python-docx v1.2.0: src/docx/opc/constants.py::CONTENT_TYPE
%   (lines 7-156)
